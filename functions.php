...
/**
 * Hale Showcase theme functions and definitions
 *
 * @package   Hale Showcase
 ...
 */
function hale_showcase_setup() {}

add_action('after_setup_theme', 'hale_showcase_setup');

function hale_showcase_enqueue_styles() {
	wp_enqueue_style( 'hale-showcase-style',
		hale_showcase_mix_asset('/css/hale-showcase-style.min.css'),
		array( 'hale-style' ),
		wp_get_theme()->get( 'Version' )
	);
	wp_enqueue_script('hale-showcase-main-script', hale_showcase_mix_asset('/js/main.js'));
}

add_action( 'wp_enqueue_scripts', 'hale_showcase_enqueue_styles' );

/**
 * Get asset path function
 * 
 * @param $filename
 * @return string
 */
function hale_showcase_mix_asset($filename)
{
